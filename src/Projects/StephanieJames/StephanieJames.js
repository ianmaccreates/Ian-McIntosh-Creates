...
import './StephanieJames.css';

const StephanieJames = () => {
  let images = [];
  try {
  ...
  return (
    <div className="sj-page">
      <h2>Stephanie James</h2>
      <p className="sj-note">Use ← and → to navigate</p>
      <div className="sj-slider">
        <Slider ref={sliderRef} {...settings}>
          {images.map((img) => (
            <div key={img.id} className="sj-slide">
              <img src={img.src} alt={img.filename} className="sj-slide-img" loading="lazy" decoding="async" />
            </div>
          ))}
        </Slider>
      </div>
