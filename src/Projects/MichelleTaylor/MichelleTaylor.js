...
import './MichelleTaylor.css';

const MichelleTaylor = () => {
  let images = [];
  try {
  ...
  return (
    <div className="mt-page">
      <h2>Michelle Taylor</h2>
      <p className="mt-note">Use ← and → to navigate</p>
      <div className="mt-slider">
        <Slider ref={sliderRef} {...settings}>
          {images.map((img) => (
            <div key={img.id} className="mt-slide">
              <img src={img.src} alt={img.filename} className="mt-slide-img" loading="lazy" decoding="async" />
            </div>
          ))}
        </Slider>
      </div>
