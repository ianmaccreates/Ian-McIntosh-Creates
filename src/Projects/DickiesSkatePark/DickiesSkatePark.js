...
import './DickiesSkatePark.css';

const DickiesSkatePark = () => {
  let images = [];
  try {
  ...
  return (
    <div className="dsp-page">
      <h2>Dickies Skate Park</h2>
      <p className="dsp-note">Use ← and → to navigate</p>
      <div className="dsp-slider">
        <Slider ref={sliderRef} {...settings}>
          {images.map((img) => (
            <div key={img.id} className="dsp-slide">
              <img src={img.src} alt={img.filename} className="dsp-slide-img" loading="lazy" decoding="async" />
            </div>
          ))}
        </Slider>
      </div>
...
export default DickiesSkatePark;
