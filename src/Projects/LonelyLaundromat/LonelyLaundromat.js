...
import './LonelyLaundromat.css';

const LonelyLaundromat = () => {
  let images = [];
  try {
  ...
  return (
    <div className="ll-page">
      <h2>Lonely Laundromat</h2>
      <p className="ll-note">Use ← and → to navigate</p>
      <div className="ll-slider">
        <Slider ref={sliderRef} {...settings}>
          {images.map((img) => (
            <div key={img.id} className="ll-slide">
              <img src={img.src} alt={img.filename} className="ll-slide-img" loading="lazy" decoding="async" />
            </div>
          ))}
        </Slider>
      </div>
...
export default LonelyLaundromat;
